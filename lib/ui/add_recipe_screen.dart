...
class _AddRecipeScreenState extends State<AddRecipeScreen> {
  ...
  // Fungsi untuk mengirim resep baru ke server
  void _submitRecipe() async {
    if (_formKey.currentState!.validate()) {
      _formKey.currentState!.save();

      // Menyiapkan data resep tanpa membuat objek RecipeModel
      try {
        final success = await _recipeService.createRecipe(
          ...
          photoPath: _selectedImage?.path ?? '', // Cek apakah gambar dipilih
        );
        ...
        if (success) {
          ...
          Navigator.pushReplacementNamed(
            context,
            "/home",
          ); // Navigasi ke halaman utama setelah menambahkan resep
        } else {
